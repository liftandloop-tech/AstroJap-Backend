...
html_block = """            {%- when 'why_astrojap_list' -%}
...
for filepath in files:
    ...
    # Remove all existing occurrences of this block HTML
    # We use escape to handle special characters in the block
    escaped_block = re.escape(html_block)
    content = re.sub(r'\s*\{%- when \'why_astrojap_list\' -%\}.*?</div>\s*</div>\s*</div>', '', content, flags=re.DOTALL)
    
    # Now find the LAST when block in the case statement and append it there
    # The block loop usually ends with {%- endcase -%}\s*{%- endfor -%}
    
    pattern = re.compile(r'(\{%\s*endcase\s*%\})', re.DOTALL)
    matches = list(pattern.finditer(content))
    if matches:
        # We want to insert BEFORE the first endcase of the block loop
        # Wait, the block loop is inside {%- for block in section.blocks -%}
        # There's usually only one big case statement in there.
        
        # Let's find the first {%- endcase -%} that follows {%- for block in section.blocks -%}
        loop_start = content.find('{%- for block in section.blocks -%}')
        if loop_start != -1:
            first_endcase_after_loop = content.find('{%- endcase -%}', loop_start)
            if first_endcase_after_loop != -1:
                content = content[:first_endcase_after_loop] + html_block + "\n            " + content[first_endcase_after_loop:]
                print(f"Correctly placed block in {filepath}")
            else:
                print(f"Could not find endcase after loop in {filepath}")
        else:
            print(f"Could not find block loop in {filepath}")
